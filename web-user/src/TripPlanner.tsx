...
export const TripPlanner: React.FC<{ user: any, API_URL: string, showAlert: (m: string, t: string) => void }> = ({ user, API_URL, showAlert }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', height: 'calc(100vh - 60px)', background: 'var(--bg-dark)' }}>
      {/* SIDEBAR */}
      <div style={{ background: '#0a0810', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        
        <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <h2 style={{ margin: '0 0 16px 0', fontSize: '20px', fontWeight: '800', color: '#FFF', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }} className="custom-scrollbar">
              ...
              {waypoints.map((wp, idx) => (
                <div key={idx} style={{ marginBottom: '16px', position: 'relative' }}>
                  ...
                  {wp.results && wp.results.length > 0 && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: '#1A1820', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', marginTop: '4px', overflow: 'hidden' }}>
                      {wp.results.map((r: any) => (
                        <div key={r.place_id} 
                          ...
                          style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '12px', color: '#FFF', cursor: 'pointer', transition: 'background 0.2s' }}
              ...
              {routeData && (
                <div style={{ marginTop: '24px', padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Distance: <b style={{ color: '#FFF' }}>{(routeData.distance / 1000).toFixed(1)} km</b></span>
                    <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>ETA: <b style={{ color: '#FFF' }}>{Math.round(routeData.duration / 60)} mins</b></span>
                  </div>
                  
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <input type="text" value={tripName} onChange={e => setTripName(e.target.value)} placeholder="Trip Name..." style={{ flex: 1, padding: '10px', background: '#121016', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', color: '#FFF', fontSize: '13px' }} />
          ...
          {activeTab === 'places' && (
            <div className="animate-fade-in">
              <h3 style={{ fontSize: '14px', color: '#FFF', marginBottom: '12px' }}>Route Filters</h3>
              ...
              <h3 style={{ fontSize: '14px', color: '#FFF', marginBottom: '12px' }}>Places Along Route ({poiData.length})</h3>
          ...
          {activeTab === 'prefs' && (
            <div className="animate-fade-in">
              <div style={{ background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '16px' }}>
                <h3 style={{ fontSize: '14px', color: '#FFF', marginBottom: '16px' }}>Vehicle Settings</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div>
                    <label style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Vehicle Type</label>
                    <select value={prefs.vehicleType} onChange={e => savePreferences({ vehicleType: e.target.value })} style={{ width: '100%', padding: '10px', background: '#121016', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', color: '#FFF', marginTop: '4px' }}>
                  ...
                  {prefs.vehicleType === 'Electric' && (
                    <div>
                      ...
                      <input type="number" value={prefs.evRange} onChange={e => savePreferences({ evRange: Number(e.target.value) })} style={{ width: '100%', padding: '10px', background: '#121016', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', color: '#FFF', marginTop: '4px' }} />
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <h3 style={{ fontSize: '14px', color: '#FFF', marginBottom: '16px' }}>Route Preferences</h3>
            ...
            <div className="animate-fade-in">
              {tripHistory.map(t => (
                <div key={t.id} style={{ background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '12px' }}>
                  <h4 style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#FFF' }}>{t.name}</h4>
                  ...
                  }} style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#FFF', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>
                    Load Itinerary
                  </button>
                </div>
              ))}
