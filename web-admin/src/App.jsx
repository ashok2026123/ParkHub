...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        const isAdmin = cleanInput === 'admin@parkeasy.in';
        
        let loggedUser;
        if (isAdmin) {
          loggedUser = {
            uid: firebaseUser.uid,
            name: firebaseUser.displayName || "Rajesh Kumar",
            email: firebaseUser.email,
            phone: firebaseUser.phoneNumber || "+91 98765 43210",
            role: "admin",
            profilePic: firebaseUser.photoURL || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200",
  ...
  const handleLoginWithCredentials = async (emailOrPhone, password) => {
    ...
    try {
      const loginEmail = emailOrPhone.includes('@') ? emailOrPhone : `${cleanInput}@parkeasy.in`;
  ...
  if (!user) {
    return <LoginScreen onLogin={handleLoginWithCredentials} onGoogleLogin={async () => { await signInWithPopup(auth, googleProvider); }} roleHint="Super Admin" />;
  }
